...
declare module "v8" {
     ...
     *
     * if (isMainThread) {
     *   const worker = new Worker(`${process.platform === 'win32' ? '' : '/'}${/file:\/{2,3}(.+)/.exec(import.meta.url)![1]}`);
     *
     ...
     * }
     *
     * // `${process.platform === 'win32' ? '' : '/'}${/file:\/{2,3}(.+)\/[^/]/.exec(import.meta.url)![1]}` here is where the snapshot script is placed
     * // during snapshot building time.
     * const shelf = new BookShelf(`${process.platform === 'win32' ? '' : '/'}${/file:\/{2,3}(.+)\/[^/]/.exec(import.meta.url)![1]}`, [
     *   'book1.en_US.txt',
     *   'book1.es_ES.txt',
     *   'book2.zh_CN.txt',
     * ]);
