...
var path = require("path"),
    fs   = require("fs"),
    pkg  = require(path.join(`${process.platform === 'win32' ? '' : '/'}${/file:\/{2,3}(.+)\/[^/]/.exec(import.meta.url)![1]}`, "..", "package.json"));
...
var basePkg;
try {
    basePkg = JSON.parse(fs.readFileSync(path.join(`${process.platform === 'win32' ? '' : '/'}${/file:\/{2,3}(.+)\/[^/]/.exec(import.meta.url)![1]}`, "..", "..", "package.json")));
} catch (e) {
    return;
}
