...
class SuiCircleBridge {
    ...
    constructor(network, chain, provider, contracts) {
        ...
        const { tokenMessengerState, messageTransmitterState, usdcTreasury } = (0, objects_js_1.suiCircleObjects)(network);
    ...
    async *transfer(sender, recipient, amount) {
        ...
        const [primaryCoin, ...mergeCoins] = await sdk_sui_1.SuiPlatform.getCoins(this.provider, sender, this.usdcId);
    ...
    async *redeem(sender, message, attestation) {
        ...
        const [receipt] = tx.moveCall({
            ...
            arguments: [
                tx.pure((0, sdk_sui_1.uint8ArrayToBCS)(sdk_connect_1.CircleBridge.serialize(message))),
                tx.pure((0, sdk_sui_1.uint8ArrayToBCS)(sdk_connect_1.encoding.hex.decode(attestation))),
                tx.object(this.messageTransmitterStateId), // message_transmitter state
            ],
        });
        if (!receipt)
    ...
    async parseTransactionDetails(digest) {
        ...
        const circleMessageSentEvent = tx.events?.find((event) => event.type.includes("send_message::MessageSent"));
        if (!circleMessageSentEvent) {
            throw new Error("No MessageSent event found");
        }
        const circleMessage = new Uint8Array((circleMessageSentEvent?.parsedJson).message);
