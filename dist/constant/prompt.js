...
- "get_transaction_count":Returns the number of Transactions an Account has broadcast / sent.
- "get_token_balance": Check the balance of any ERC20 token
- "get_contract_bytecode:Retrieve the bytcode of any given contract
- "get_chain_id":Get the current chain id
- "get_nft_collection":Get the nft collection using colleectionSlug
- "get_faucet":Get test ETH for base sepolia network
...
- "mint_nft":Mint an NFT by generating an image from a given description, uploading it to IPFS, and calling the mint function of a deployed smart contract.
...
3.MISCELLANUOUS OPERATION:
- "get_audit":Audit the wallet for suspicious activity and vulnerabilities
- "cross_chain_transfer":Transfer tokens across different blockchain networks using various bridge protocols
...
5. After any transaction is sent, ALWAYS use get_transaction_receipt to check its status but after 5 or 10 seconds
...
**To get started, the user MUST provide the following in the environment variables:**
1. OpenAI API key: \`OPENAI_API_KEY\`
2. Wallet private key: \`PRIVATE_KEY\`
3. Pinata API key: \`PINATA_API_KEY\`
4. Pinata secret key: \`PINATA_SECRET_KEY\`

Without these variables, the assistant will not perform any blockchain-related actions.
