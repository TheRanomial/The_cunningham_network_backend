...
(async function () {
    ...
    const vaa = await wh.getVaa("07D6A246859FAB4A93FFF2AE1F3B4CB794754226560F3886BE01A39F3204E148", "Uint8Array");
...
export function repairVaa(vaa, guardianSetData) {
    if (vaa.guardianSet === guardianSetData.index)
    ...
    const validSignatures = vaa.signatures.filter((signature) => {
        try {
            return !encoding.bytes.equals(currentGuardianSet[signature.guardianIndex], SignatureUtils.recover(signature.signature, digest));
        }
        catch (_) { }
        return false;
    });
