...
(async function () {
    ...
    const token = Wormhole.tokenId(external.chain, "native");
    ...
    let route2 = fakeIt
        ...
        : await transferBetweenCosmos(wh, { chain: cosmos1.chain, address: cosmosTokenAddress }, 1000n, leg2, leg3);
    ...
    let route3 = fakeIt
        ...
        : await transferOutOfCosmos(wh, { chain: cosmos2.chain, address: cosmosTokenAddress }, 1000n, leg3, leg1);
...
async function transferIntoCosmos(wh, token, amount, src, dst) {
    ...
    console.log(`Beginning transfer into Cosmos from ${src.chain.chain}:${src.address.address.toString()} to ${dst.chain.chain}:${dst.address.address.toString()}`);
...
async function transferBetweenCosmos(wh, token, amount, src, dst) {
    // EXAMPLE_GATEWAY_INTERCOSMOS
    console.log(`Beginning transfer within cosmos from ${src.chain.chain}:${src.address.address.toString()} to ${dst.chain.chain}:${dst.address.address.toString()}`);
    const xfer = await GatewayTransfer.from(wh, {
        token: token,
        amount: amount,
        from: src.address,
...
async function transferOutOfCosmos(wh, token, amount, src, dst) {
    // EXAMPLE_GATEWAY_OUTBOUND
    console.log(`Beginning transfer out of cosmos from ${src.chain.chain}:${src.address.address.toString()} to ${dst.chain.chain}:${dst.address.address.toString()}`);
    const xfer = await GatewayTransfer.from(wh, {
        token: token,
        amount: amount,
        from: src.address,
