...
const chainRPCs = {
    sepolia: "https://eth-sepolia.g.alchemy.com/v2/vg6DD9GmduZ75YH8EMj0A5H8EwwZd61m",
...
export const crossChainTransferTool = {
    definition: {
        type: "function",
        function: {
            name: "cross_chain_transfer",
            description: "Transfer tokens across different blockchain networks using various bridge protocols",
            parameters: {
                type: "object",
                properties: {
                    sourceChain: {
                        type: "string",
                        description: "Source blockchain network (e.g., 'ethereum', 'polygon')",
                    },
                    destinationChain: {
                        type: "string",
    ...
    handler: async (args) => {
        try {
            ...
            const bridgeConfig = BRIDGE_CONFIGS[args.bridgeProtocol][args.sourceChain];
            if (!bridgeConfig) {
                throw new Error(`Bridge protocol ${args.bridgeProtocol} not supported for ${args.sourceChain}`);
            }
            const chainIds = {
                ethereum: 1,
                sepolia: 11155111,
            ...
            if (!isValidChain(args.destinationChain)) {
                throw new Error(`Unsupported destination chain: ${args.destinationChain}`);
            }
            ...
            if (!bridgeConfig.supportedChains.includes(destinationChainId)) {
                throw new Error(`Destination chain ${args.destinationChain} not supported for this bridge configuration`);
            }
            const account = privateKeyToAccount("0xd82a39ab849fa5fa2b9c4f2acd7260a9af7c8e593155c4cca255e927700ab762");
            ...
            console.log("Adjusted amount:", formatUnits(adjustedAmount, decimals));
            if (balance < adjustedAmount) {
                throw new Error(`Insufficient token balance. Required: ${args.amount}, Available: ${formatUnits(balance, decimals)}`);
            }
            // Check allowance
            const currentAllowance = await publicClient.readContract({
                ...
                args: [account.address, bridgeConfig.bridgeAddress],
            });
            ...
            if (currentAllowance < adjustedAmount) {
                ...
                const { request: approveRequest } = await publicClient.simulateContract({
                    ...
                    args: [bridgeConfig.bridgeAddress, adjustedAmount],
                });
                ...
                console.log("Token approved. Receipt:", approveReceipt);
            }
            ...
            const { request: bridgeRequest } = await publicClient.simulateContract({
                account,
                address: bridgeConfig.bridgeAddress,
                abi: bridge_abi,
                functionName: "bridge",
                args: [
                    args.tokenAddress,
            ...
            console.log("Bridge transaction hash:", bridgeHash);
            const receipt = await publicClient.waitForTransactionReceipt({
                hash: bridgeHash,
            });
            console.log("Bridge transaction completed. Receipt:", receipt);
            return {
                status: "success",
                ...
                amount: args.amount,
                estimatedTime: getBridgeTime(args.bridgeProtocol, args.sourceChain, args.destinationChain),
                trackingUrl: generateTrackingUrl(args.bridgeProtocol, receipt.transactionHash),
            };
        }
        catch (error) {
...
// Add `console.log` in helper functions if needed.
function getBridgeTime(protocol, sourceChain, destChain) {
    const protocolTimes = bridgeTimes[protocol];
    if (typeof protocolTimes[sourceChain] === "object") {
        const sourceChainTimes = protocolTimes[sourceChain];
        return sourceChainTimes[destChain] ?? protocolTimes.default;
    }
    return protocolTimes.default;
}
function generateTrackingUrl(protocol, txHash) {
    const trackingUrls = {
