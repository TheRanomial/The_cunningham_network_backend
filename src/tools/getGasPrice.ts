...
interface GasPriceOptimizationArgs {
  userCeiling: string;
}

export const gasPriceOptimizationTool: ToolConfig<GasPriceOptimizationArgs> = {
  definition: {
    type: "function",
    function: {
      name: "optimize_gas_price",
      description:
        "Fetch real-time gas prices and recommend an optimized gas price based on user-defined ceiling.",
      parameters: {
        type: "object",
        properties: {
          userCeiling: {
            type: "string",
            description:
              "The maximum gas price (in Gwei) the user is willing to pay.",
          },
        },
  ...
  handler: async ({ userCeiling }) => {
    try {
      ...
      const optimizedGasPrice = getOptimizedGasPrice(
        gasPrices,
        userCeilingInWei
      );
...
function getOptimizedGasPrice(
  prices: { low: bigint; average: bigint; high: bigint },
  userCeiling: bigint
) {
  return prices.average < userCeiling ? prices.average : userCeiling;
}

function formatGasPrice(priceInWei: bigint) {
