...
export const mintNftTool: ToolConfig<MintNftArgs> = {
  definition: {
    ...
    function: {
      name: "mint_nft",
      description:
        "Generate a image from the given description and then mint NFT from it",
      parameters: {
        type: "object",
        properties: {
          imageDescription: {
...
async function mintnft(
  ...
  try {
    ...
    const nftTransaction = await mintToWallet(
      wallet,
      ipfsHash,
      contractAddress
    );
...
async function uploadToIpfs(imageUrl: string, imageDescription: string) {
